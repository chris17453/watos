...
use super::AhciDriver;
use watos_driver_framework::block::BlockDevice;
use watos_driver_ahci::DiskInfo;
...
/// Wrapper around AhciDriver that provides legacy API
struct DiskWrapper {
    driver: AhciDriver,
}
...
impl DiskWrapper {
    ...
    /// Read sectors using legacy (lba, count, buffer) API
    fn read_sectors(&mut self, lba: u64, _count: u16, buffer: &mut [u8]) -> bool {
        self.driver.read_sectors(lba, buffer).is_ok()
    }

    /// Write sectors using legacy (lba, count, buffer) API
    fn write_sectors(&mut self, lba: u64, _count: u16, buffer: &[u8]) -> bool {
        self.driver.write_sectors(lba, buffer).is_ok()
    }

    /// Get disk info
    fn identify(&mut self) -> Option<DiskInfo> {
        self.driver.identify().ok()
    }
}
...
impl Wfs {
    ...
    pub fn try_mount(driver: AhciDriver) -> MountResult {
        let mut disk = DiskWrapper::new(driver);
    ...
    pub fn mount(driver: AhciDriver) -> Option<Self> {
        match Self::try_mount(driver) {
    ...
    pub fn format(driver: AhciDriver, max_files: u32) -> Option<Self> {
        let mut disk = DiskWrapper::new(driver);
