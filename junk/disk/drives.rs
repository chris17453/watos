...
extern crate alloc;
use super::{AhciDriver, BoxedFs, create_vfs};
use super::wfs::Wfs;
...
/// Create and initialize an AHCI driver for a specific port
pub fn create_ahci(port: u8) -> Option<AhciDriver> {
...
impl DriveManager {
    ...
    pub fn scan_and_mount(&mut self) -> u8 {
        ...
        // Scan AHCI ports 0-5
        for port in 0..6u8 {
            if let Some(mut ahci) = create_ahci(port) {
                use watos_driver_framework::block::BlockDevice;
                if let Ok(info) = ahci.identify() {
                    // Read first sector to check for partition table
                    let mut sector = [0u8; 512];
                    if ahci.read_sectors(0, &mut sector).is_err() {
                        continue;
                    }
                    ...
                    match pt.table_type {
                        ...
                        PartitionTableType::Gpt => {
                            ...
                            if ahci.read_sectors(1, &mut gpt_header).is_err() {
                                continue;
                            }
                            ...
                            for sector_idx in 0..sectors_needed {
                                let mut sector = [0u8; 512];
                                if ahci.read_sectors(partition_entry_lba + sector_idx as u64, &mut sector).is_err() {
                                    break;
                                }
    ...
    fn detect_filesystem(&self, ahci: &mut AhciDriver) -> FsType {
        self.detect_filesystem_at(ahci, 0)
    }

    /// Detect filesystem type at a specific LBA offset
    fn detect_filesystem_at(&self, ahci: &mut AhciDriver, lba: u64) -> FsType {
        use watos_driver_framework::block::BlockDevice;
        let mut sector = [0u8; 512];

        if ahci.read_sectors(lba, &mut sector).is_err() {
    ...
    pub fn format_wfs_by_name(&mut self, name: &str) -> bool {
        ...
        if let Some(ahci) = create_ahci(port) {
