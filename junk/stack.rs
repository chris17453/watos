...
//! Uses the kernel driver module for NIC access.
...
impl NetworkStack {
    /// Create a new network stack using the kernel driver
    pub fn new() -> Option<Self> {
        // Get MAC from driver
        let mac = drivers::get_mac_address()?;
    ...
    fn send(&self, buffer: &[u8]) {
        let _ = drivers::send_packet(buffer);
    }

    /// Receive a raw packet
    fn recv(&self, buffer: &mut [u8]) -> usize {
        drivers::receive_packet(buffer).unwrap_or(0)
    }
...
impl Default for NetworkStack {
    fn default() -> Self {
        Self::new().expect("No network device available")
    }
}
